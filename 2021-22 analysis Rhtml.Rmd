...
```{r}
rawdata <- read.csv("./Newdata/Allsurveys18 3.csv", row.names = 1) #retain 'rawdata' as an unmodified waterbird import
...
ggplot(data = rawdata2) + geom_col(mapping = aes(x=reorder(date2, order), y = RICHNESS, fill = Period)) + xlab("Survey") + ylab("Number of waterbird species") + theme(axis.title.x = element_text(margin=margin(t=20))) + theme(axis.title.y = element_text(margin=margin(r=20))) +theme(axis.text=element_text(size=10), axis.title=element_text(size=14,face="bold")) + theme(axis.text.x = element_text(angle = 90))
```
...
rawdata3.T$colour <- rawdata3.T$All_Ref #new variable (colour) for depth and completeness of surveys
...
rawdata.T.SA <- rawdata.T.SA[, -c(1:7,50:51)] #delete columns with factors
...
rawdata.T.SA2 <- rawdata.T.SA2[, -c(1:7,50:51)]
...
rawdata.T.SA3 <- rawdata.T.SA3[, -c(1:5,48:50)]
rawdata.T.SA3 <- rawdata.T.SA3[-grep("TM", rownames(rawdata.T.SA3)), ]
rawdata.T.SA3 <- rawdata.T.SA3 %>% replace(is.na(.), 0)
...
####Ordination analyses with just surveys that were considered complete and depth >=0.6m

```{r eval = FALSE}
comdataA <- rawdata[rawdata$RICHNESS >1 & rawdata$ALL_SPECIES=="A" & rawdata$DEPTH > 0.599, ]
...
####Cluster analysis of just surveys considered complete
...
<!-- conditions, the next analyses use only surveys where at least 10 species were seen (indicating a reasonable survey effort) and depth was moderate to high (\>0.3m). -->

<!-- ###Analyses with surveys that recorded 10 or more species and depth >=0.6m -->
...
<!-- There is not a clear pattern here of composition being related to depth. The plot still shows all of the 1996-2022 surveys in one part of the ordination and on the edge of the 1980s surveys, but not clearly -->
<!-- separated from them. -->
...
<!-- comdataR.fact$APeriod <- str_c(comdataR.fact$ALL_SPECIES, comdataR.fact$Period) -->
<!-- comdataR.fact$APeriod.col <- comdataR.fact$APeriod -->
<!-- comdataR.fact$APeriod.col <- gsub("A1996|A2017|A2021-22", 'red', comdataR.fact$APeriod.col) -->
<!-- comdataR.fact$APeriod.col <- gsub("A1980s|A1975", 'green', comdataR.fact$APeriod.col) -->
<!-- comdataR.fact$APeriod.col <- gsub("N1980s", 'grey', comdataR.fact$APeriod.col) -->
<!-- #plot(comdataR.upgma, hang=-1) -->
<!-- plotColoredClusters(comdataR.upgma, comdataR.fact$VISIT_DATE, comdataR.fact$APeriod.col, cex = 0.8, main = "", line = 0) -->
<!-- ``` -->
...
comdataRT <- comdataRT[, -(1:7)] #remove factors from data file
...
ggplot(mds5.pts, aes(x=MDS1, y=MDS2)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(aes(colour=Period), size=6, alpha=0.8) + geom_point(aes(pch = All_Ref), data = mds5.pts[mds5.pts$All_Ref == "Y", ]) + coord_fixed(ratio = 1)
```

##Heatmap of species abundances by comprehensive survey
```{r}
comdataHM <- rawdata[c(rawdata$All_Ref == "Y"), ] #selects reference surveys
...
<!-- ###Repeated with only sites well surveyd with depth >=0.6m -->

<!-- ```{r} -->
<!-- comdataRTA <- rawdata[rawdata$ALL_SPECIES == "A" & rawdata$DEPTH > 0.599, ] -->
<!-- comdataRTA[is.na(comdataRTA)]<-0 # replace na with 0 -->
<!-- comdataRTA <- comdataRTA[grep("^T", row.names(comdataRTA)),] #extracts Toolibin -->
<!-- comdataRTA.fact <- comdataRTA[, 1:5] -->
<!-- comdataRTA.fact$Survey <- row.names(comdataRTA.fact) -->
<!-- comdataRTA.fact$YM <- format(as.Date(comdataRTA.fact$VISIT_DATE), "%m-%y") -->
<!-- comdataRTA.fact$Period2 <- comdataRTA.fact$Period -->
<!-- comdataRTA.fact$Period2 <- gsub('1996', '1996-2022', comdataRTA.fact$Period2) -->
<!-- comdataRTA.fact$Period2 <- gsub('2017', '1996-2022', comdataRTA.fact$Period2) -->
<!-- comdataRTA.fact$Period2 <- gsub('2021-22', '1996-2022', comdataRTA.fact$Period2) -->
<!-- comdataRTA.fact$Period2 <- gsub('1980s', 'pre-1996', comdataRTA.fact$Period2) -->
<!-- comdataRTA <- comdataRTA[, -(1:5)] #remove factors from data file -->
<!-- comdataRTA <- comdataRTA[, colSums(comdataRTA) != 0] -->
<!-- comdataRTA.pa<-decostand(comdataRTA,"pa") #create presence/absence matrix -->
<!-- comdataRTA.hel<-decostand(comdataRTA,"hellinger") # create helinger transformed matrix -->
<!-- comdataRTA.sqrt <- sqrt(comdataRTA) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- row.names(comdataRTA.hel) <- comdataRTA.fact$VISIT_DATE -->
<!-- melt_comdataRTA.hel <- melt(as.matrix(comdataRTA.hel)) -->
<!-- ggplot(melt_comdataRTA.hel, aes(Var2, Var1)) + geom_tile(aes(fill = value), colour = "white") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + -->
<!--   scale_fill_gradient(low = "lightblue", high = "red") + labs(x="Species",y="Survey") -->
<!-- ``` -->

<!-- ```{r} -->
<!-- comdataRTA.Simper <- simper(comdataRTA.hel, comdataRTA.fact$Period2) -->
<!-- comdataRTA.Simper -->
<!-- summary(comdataRTA.Simper) -->
<!-- ``` -->

<!-- ```{r include=FALSE} -->
<!-- mds6<-metaMDS(comdataRTA.hel, distance="bray", k=3) #nMDS in 3 dimensions -->
<!-- ``` -->

<!-- ```{r} -->
<!-- #Extract mds points and add factors -->
<!--   mds6.pts <- as.data.frame(mds6$points) #extract 2D ordination coordinates (points) -->
<!--   mds6.pts$Survey <- row.names(comdataRTA.fact) #use survey period as row.names -->
<!--   mds6.pts$YM <- comdataRTA.fact$YM #add YearMonth for labelling -->
<!--   mds6.pts$Period <- comdataRTA.fact$Period -->
<!--   mds6.pts$Depth <- comdataRTA.fact$DEPTH -->
<!--   #mds5.pts$Period <- comdataRT.fact[match(row.names(mds5.pts), row.names(comdataRT.fact)), 2] -->
<!--   #mds5.pts$Depth <- comdataRT.fact[match(row.names(mds5.pts), row.names(comdataRT.fact)), 4] -->
<!-- ``` -->
...
<!-- ggplot(mds6.pts, aes(x=MDS1, y=MDS2)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(aes(colour=Period), size=6, alpha=0.5) + coord_fixed(ratio = 1) + geom_text(aes(label=YM), size=3, vjust=1, hjust=1.3) -->
<!-- ``` -->
...
comdataRT.fact$Period2 <- gsub('1975', 'pre-1996', comdataRT.fact$Period2)
comdataRT.fact$Period2 <- gsub('1980s', 'pre-1996', comdataRT.fact$Period2)
comdataRT.Simper <- simper(comdataRT, comdataRT.fact$Period2)
summary(comdataRT.Simper)
str(comdataRT.Simper)
```

<!-- ###Produce heatmap with surveys clustered by Kmeans -->
...
<!-- comdataRT.fact$kmgrpsRT2 <- gsub(G, 'Group one',comdataRT.fact$kmgrpsRT2) -->
...
<!-- KMgrpsRT.2grp <- data.frame(matrix(ncol = 1, nrow = 40)) -->
...
<!-- This Simper analyses suggests that the abundances of the following 13 species best distinguish kmeans group `r G` from the remaining sites -->
...
<!-- # ###heatmap with only all six kmeans groups -->
<!-- # -->
<!-- # ```{r warning=FALSE} -->
...
###histograms of abundance of species identified in the Simper analysis
