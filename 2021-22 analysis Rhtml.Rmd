...
packages <- c("RColorBrewer","stringr","rstudioapi","patchwork","glue","ClassDiscovery","vegan","tidyverse","lubridate","ggplot2","ggpubr","stringi","factoextra", "zoo","cluster","reshape2","ggthemes","data.table")
...
The waterbird data input files include depth at time of survey, visit
date, period (1980s, 1996, 2017 and 2021/22), an indication
(ALL_SPECIES) of whether the observers thought they had adequately
surveyed the whole of the lake and seen all species, and species
richness.
...
##Column plots for richness - using only survey dates where observers
thought all species counted and depth \>=0.3m
...
##scatterplots by period with colour dem=noting whether all species
likely observed
...
All multivariate analyses are performed on Helinger transformed
abundance data (within species abundances scaled to values between 0 and
1 and square root transformed).
...
A two-dimensional ordination had unacceptably high stress value
(`r round(mds1$stress, digits=2)`) so a 3 dimensional ordination was run
with stress `r round(mds2$stress, digits=2)`
...
Below is a plot of axes 1 v 2 of the 3D ordination, showing all of the
1996 to 2022 surveys on the edge of the 1980s surveys.
...
colnames(Taxonomy3) <- c("Group", "Code", "LowestID")
...
ggplot(Inv.stack.T.agg, aes(x=reorder(Sample, Order), y=Presence, fill=Group)) + geom_bar(stat="identity", colour = "black") +  scale_fill_brewer(palette="Set3")
