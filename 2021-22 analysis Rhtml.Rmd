...
packages <- c("RColorBrewer","stringr","rstudioapi","patchwork","glue","ClassDiscovery","vegan","tidyverse","lubridate","ggplot2","ggpubr","stringi","factoextra", "zoo","cluster","rgl","reshape2","ggthemes","data.table")
...
mds3.I.pts$Wetland <- as.factor(Inv.fact2[match(row.names(mds3.I.pts), row.names(Inv.fact2)), 3])
mds3.I.pts$Order <- as.factor(Inv.fact2[match(row.names(mds3.I.pts), row.names(Inv.fact2)), 4])
mds3.I.pts$Col <- as.string(mds3.I.pts$Wetland)
...
ggplot(mds3.I.pts, aes(x=MDS2, y=MDS3)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(aes(colour=Wetland, pch = Survey), size=5) + coord_fixed(ratio = 1) + theme(text = element_text(size=15))
#+ geom_text(aes(label=Sample), size=2, vjust=1, hjust=1.3) #add labels if needed
```

```{r}
install.packages("rgl")
plot3d(x = mds3.I.pts$MDS1, y = mds3.I.pts$MDS2, z = mds3.I.pts$MDS3, col = mds3.I.pts$Wetland)
```
