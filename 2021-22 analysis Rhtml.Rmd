...
All multivariate analyses are performed on Helinger transformed abundance data (within species abundances scaled to values between 0 and 1 and square root transformed). Surveys with richness <=1 removed to deal with an outlier identified in the 2019 analyses.
...
ggplot(mds5.pts, aes(x=MDS1, y=MDS2)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(aes(colour=Period), size=6, alpha=0.8) + geom_point(aes(pch = AvN), data = mds5.pts[mds5.pts$AvN == "A D", ]) + coord_fixed(ratio = 1)
```
...
comdataRTA.fact$Period2 <- gsub('1975', 'pre-1996', comdataRTA.fact$Period2)
