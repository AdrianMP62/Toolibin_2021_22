...
Inv.stack.T.G.agg <- aggregate(data=Inv.stack.T.G, Presence ~ Period + Group, FUN = "max") #aggregate count data by species and survey
...
ggplot(Inv.stack.T.G.agg, aes(x=reorder(Period, Order), y=Presence, fill=Grouporder)) + 
  ...
  theme(axis.title.y = element_text(vjust = +2.5)) +
  theme(legend.title=element_blank()) + 
    geom_vline(xintercept = c(1.5, 2.5, 3.5, 4.5), color = "black", linetype = "solid")
```
