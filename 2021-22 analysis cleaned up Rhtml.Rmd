...
comdataRTA.guilds <- comdataRTA.guilds[!rownames(comdataRTA.guilds) == "T1075", ] #removes 1975 presence absence data 
...
##Scatterplots of guild richness by guild
```{r}
ggplot(comdataRTA.guilds, aes(Period, Richness)) +
  geom_jitter(width = 0.0, height = 0.0, size=5, pch=16, colour="blue", alpha = 0.4) + scale_x_discrete(limits = c("Reference", "1996-2022")) +
...
###boxplots for major feeding guilds
```{r}
guilds <- read.csv("./Newdata/ToolibinGuilds.csv")
comdataHMG <- comdataHM
names(comdataHMG) <- guilds$Guild[match(names(comdataHMG), guilds$Species)]
comdataHMG[1:5, ]
```
