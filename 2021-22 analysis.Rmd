---
title: "ANALYSES OF TOOLIBIN AND WALBYRING WATERBIRD COUNTS and INVERTEBRATES INCLUDING 2021-22"
...
packages <- c("gridExtra","dendextend","bibliometrix","data.table","dplyr","scatterplot3d","plotly","RColorBrewer","stringr","rstudioapi","patchwork","glue","ClassDiscovery","vegan","tidyverse","lubridate","ggplot2","ggpubr","stringi","factoextra", "zoo","cluster","rgl","reshape2","ggthemes","data.table")
...
Analyses below are for a report by Pinder et al.on 2021/22 surveys of invertebrates and waterbirds
...
##Stacked waterbird abundance column plots by group

```{r}
#Toolibin Lake
Toolibin.stack <- rawdata[grep("^T", row.names(rawdata)),] #import waterbird data and extract Toolibin surveys
Toolibin.stack <- Toolibin.stack[Toolibin.stack$All_Ref %in% c("Y","M"), ] #restricted to 1996 to 2022 plus reference surveys
Toolibin.stack <- Toolibin.stack[!rownames(Toolibin.stack) == "T1075", ]
Toolibin.stack <- subset(Toolibin.stack, select = -c(Period, Period2, ALL_SPECIES, FHS_Ref, All_Ref, DEPTH, RICHNESS)) #remove unwanted columns
Toolibin.stack <- Toolibin.stack[order(Toolibin.stack$VISIT_DATE), ] #ensure correctly sorted by date before adding an order variable
Toolibin.stack <- cbind(order = 1:nrow(Toolibin.stack), Toolibin.stack) #add an order variable for dates
Toolibin.stack <- melt(data = Toolibin.stack, id.vars = c("VISIT_DATE","order"), variable.name = "Species", value.name = "Abundance") #convert to long form
Toolibin.stack <- Toolibin.stack[complete.cases(Toolibin.stack), ] #remove rows with missing values
ToolibinGuilds <- read.csv("./Newdata/ToolibinGuilds.csv")
Toolibin.stack$Group <- ToolibinGuilds[match(Toolibin.stack$Species, ToolibinGuilds$Species), "Group"] #add column for taxonomic group
Toolibin.stack$Guild <- ToolibinGuilds[match(Toolibin.stack$Species, ToolibinGuilds$Species), "Guild"] #add column for taxonomic group
```


```{r}
#Aggregate data by taxonomic group and survey, with survey and season variables for plotting
Toolibin.stack.agg.group <- aggregate(data=Toolibin.stack, Abundance ~ Group + order +VISIT_DATE, FUN="sum") #aggregate count
Toolibin.stack.agg.guild <- aggregate(data=Toolibin.stack, Abundance ~ Guild + order +VISIT_DATE, FUN="sum") #aggregate count
```

```{r}
#stacked column plots for Toolibin Lake
ggplot(Toolibin.stack.agg.group, aes(x=reorder(VISIT_DATE, order), y=Abundance, fill=Group)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,7000, by=2000), limits=c(0,7000)) + labs(x="Survey", y="Abundance") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(size = 15, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(size = 15, margin = margin(t = 20, r = 0, b = 0, l = 0))) + theme(axis.text.x = element_text(size=12)) + theme(axis.text.y = element_text(size=15)) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF","#B4CDCD","#C1FFc1","#A52A2A"))
#plot by guild
ggplot(Toolibin.stack.agg.guild, aes(x=reorder(VISIT_DATE, order), y=Abundance, fill=Guild)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,7000, by=2000), limits=c(0,7000)) + labs(x="Survey", y="Abundance") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(size = 15, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(size = 15, margin = margin(t = 20, r = 0, b = 0, l = 0))) + theme(axis.text.x = element_text(size=12)) + theme(axis.text.y = element_text(size=15)) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF","#B4CDCD","#C1FFc1","#A52A2A"))
```


```{r}
#Walbyring Lake
Walbyring.stack <- rawdata[grep("^W", row.names(rawdata)),] #import waterbird data and extract Walbyring surveys
Walbyring.stack <- Walbyring.stack[Walbyring.stack$RICHNESS >4, ]
Walbyring.stack <- subset(Walbyring.stack, select = -c(Period, Period2, ALL_SPECIES, FHS_Ref, All_Ref, DEPTH, RICHNESS)) #remove unwanted columns
Walbyring.stack <- Walbyring.stack[order(Walbyring.stack$VISIT_DATE), ] #ensure correctly sorted by date before adding an order variable
Walbyring.stack <- cbind(order = 1:nrow(Walbyring.stack), Walbyring.stack) #add an order variable for dates
Walbyring.stack <- melt(data = Walbyring.stack, id.vars = c("VISIT_DATE","order"), variable.name = "Species", value.name = "Abundance") #convert to long form
Walbyring.stack <- Walbyring.stack[complete.cases(Walbyring.stack), ] #remove rows with missing values
WalbyringGuilds <- read.csv("./Newdata/ToolibinGuilds.csv")
Walbyring.stack$Group <- WalbyringGuilds[match(Walbyring.stack$Species, WalbyringGuilds$Species), "Group"] #add column for taxonomic group
Walbyring.stack$Guild <- WalbyringGuilds[match(Walbyring.stack$Species, ToolibinGuilds$Species), "Guild"] #add column for taxonomic group
```


```{r}
#Aggregate data by taxonomic group and survey, with survey and season variables for plotting
Walbyring.stack.agg.group <- aggregate(data=Walbyring.stack, Abundance ~ Group + order +VISIT_DATE, FUN="sum") #aggregate count
Walbyring.stack.agg.guild <- aggregate(data=Walbyring.stack, Abundance ~ Guild + order +VISIT_DATE, FUN="sum") #aggregate count
```

```{r}
#stacked column plots for Walbyring Lake
ggplot(Walbyring.stack.agg.group, aes(x=reorder(VISIT_DATE, order), y=Abundance, fill=Group)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,1250, by=250), limits=c(0,1250)) + labs(x="Survey", y="Abundance") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(size = 15, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(size = 15, margin = margin(t = 20, r = 0, b = 0, l = 0))) + theme(axis.text.x = element_text(size=12)) + theme(axis.text.y = element_text(size=15)) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#FFE1FF","#B4CDCD"))
#plot by guild
ggplot(Walbyring.stack.agg.guild, aes(x=reorder(VISIT_DATE, order), y=Abundance, fill=Guild)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,1250, by=250), limits=c(0,1250)) + labs(x="Survey", y="Abundance") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(size = 20, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(size = 20, margin = margin(t = 20, r = 0, b = 0, l = 0))) + theme(axis.text.x = element_text(size=12)) + theme(axis.text.y = element_text(size=15)) + scale_fill_manual(values = c("#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF"))
```
...
rawdata.I <- rawdata.I.orig[, 8:37] #use only Toolibin-Walbyring data (exclude SPS sites and species names)
...
ggplot(mds1.I.pts, aes(x=MDS1, y=MDS2, colour = Wetland)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) +
  geom_point(aes(pch = Survey), size=7) +
  coord_fixed(ratio = 1) + theme(text = element_text(size=15)) +
  scale_colour_manual(values = c("lightblue","darkblue")) + scale_shape_manual(values = c(20,15,17,18,16)) 
#+ geom_text(aes(label=Sample), size=2, vjust=1, hjust=1.3) #add labels if needed
```

## Multivariate analyses including five reference sites and 6 degraded sites from Pinder, A.M., S.A. Halse, J.M. McRae, and R. J. Shiel. ‘Aquatic Invertebrate Assemblages of Wetlands and Rivers in the Wheatbelt Region of Western Australia’. Records of the Western Australian Museum Supplement 67 (2004): 7–37.

```{r include=FALSE}
#create datasets
rawdata.I2 <- rawdata.I.orig[, 2:42] #remove species names
...
ggplot(mds2.I.pts, aes(x=MDS1, y=MDS2, shape = Survey)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(aes(colour=Wetland), size=5) + coord_fixed(ratio = 1) + theme(text = element_text(size=15)) + scale_shape_manual(values = c(1,2,3,4,5,6,7))
#+ geom_text(aes(label=Sample), size=2, vjust=1, hjust=1.3) #add labels if needed
```

```{r include=FALSE}
#re-run ordination without the April 2017 samples
rawdata.I3 <- rawdata.I.orig[, 2:42] #remove species names
...
Inv.fact.orig <- read.csv("./Newdata/Invert factors.csv", row.names = 1) #import site factor variables
Inv.fact3 <- Inv.fact.orig[-grep("A17", Inv.fact.orig$Survey), ]
#rawdata.I[is.na(rawdata.I)]<-0 # replace na with 0
```
...
ggplot(mds3.I.pts, aes(x=MDS1, y=MDS2, colour = Wetland, shape = Survey)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(size=5) + coord_fixed(ratio = 1) + theme(text = element_text(size=15)) + scale_colour_manual(values = c("red","green","lightblue","darkblue")) + scale_shape_manual(values = c(15,16,17,18,19,16))
#+ geom_text(aes(label=Sample), size=2, vjust=1, hjust=1.3) #add labels if needed
```


<!-- Unused code not used in report to create a 3D ordination plot with lines. This does not work with the degraded sites included -->
<!-- ```{r} -->
...
<!-- mds3.I.pts['wetland'][mds3.I.pts['wetland'] == 'Degraded'] <- '4' -->
...
<!-- plot_ly(color = ~factor(wetland), colors = c("orange", "blue", "red","green"), showlegend = T) %>% -->
